...
$grand=0;
 ...
 $sql = DB::table('journal')
            ...
            ->whereIn('ChartOfAccountID',$chart)
...
else
{
 ...
 ?>
  <tr>
    <td>{{$value->CategoryCode}}-{{$value->PartyName}}</td>
...
$opening_bal = DB::table('v_vc_montly_balance')->where('PartyID',$value->PartyID)->where('Date',$date)->get();
}
